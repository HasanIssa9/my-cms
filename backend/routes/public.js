...
// ─── Responsive CSS (embedded once per page) ──────────────────────────────────
const GLOBAL_CSS = `
<style>
/* ── Reset & base ── */
...
@keyframes bounceIn  {0%{opacity:0;transform:scale(0.3)}55%{opacity:1;transform:scale(1.06)}75%{transform:scale(0.96)}100%{transform:scale(1)}}

.cb{opacity:0}
.cb.vis[data-a="fadeInUp"]  {animation:fadeInUp   0.7s cubic-bezier(.22,.68,0,1.2) both}
.cb.vis[data-a="fadeIn"]    {animation:fadeIn     0.7s ease both}
.cb.vis[data-a="slideLeft"] {animation:slideLeft  0.7s cubic-bezier(.22,.68,0,1.2) both}
.cb.vis[data-a="slideRight"]{animation:slideRight 0.7s cubic-bezier(.22,.68,0,1.2) both}
.cb.vis[data-a="zoomIn"]    {animation:zoomIn     0.7s cubic-bezier(.22,.68,0,1.2) both}
.cb.vis[data-a="bounceIn"]  {animation:bounceIn   0.8s ease both}
.cb[data-a="none"]          {opacity:1!important;animation:none!important}

/* ── Site nav ── */
.s-nav{background:#1e293b;padding:0 1.5rem;display:flex;align-items:center;justify-content:space-between;min-height:60px;position:sticky;top:0;z-index:100;box-shadow:0 2px 8px rgba(0,0,0,0.2)}
.s-logo{color:#60a5fa;font-size:1.2rem;font-weight:800;text-decoration:none;flex-shrink:0}
.s-links{display:flex;gap:1.5rem;align-items:center}
.s-links a{color:rgba(255,255,255,0.75);text-decoration:none;font-size:0.9rem;transition:color 0.15s}
.s-links a:hover{color:#fff}
.s-burger{display:none;background:none;border:none;cursor:pointer;padding:4px;color:#fff;font-size:1.4rem}
.s-mobile-menu{display:none;position:fixed;inset:60px 0 0 0;background:#1e293b;z-index:99;padding:1.5rem;flex-direction:column;gap:1rem}
.s-mobile-menu.open{display:flex}
.s-mobile-menu a{color:rgba(255,255,255,0.85);font-size:1.1rem;text-decoration:none;padding:0.5rem 0;border-bottom:1px solid #334155}

/* ── Page layout ── */
.p-wrap{max-width:900px;margin:2rem auto;border-radius:14px;box-shadow:0 2px 32px rgba(0,0,0,0.09);overflow:hidden}
.p-inner{padding:var(--page-pad,2rem)}
.p-foot{text-align:center;padding:1.5rem;color:#64748b;font-size:0.82rem;margin-top:0.5rem}

/* ── Block types ── */
.b-hero{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:var(--hero-h,50vh);background-size:cover;background-position:center;position:relative;overflow:hidden;text-align:center}
.b-hero-inner{position:relative;z-index:1;max-width:720px;padding:2rem}
.b-hero h1{font-size:clamp(1.8rem,5vw,3rem);font-weight:800;line-height:1.15;margin-bottom:0.75rem}
.b-hero p{font-size:clamp(0.95rem,2vw,1.15rem);opacity:0.88;margin-bottom:1.5rem}
.b-hero a{display:inline-block;padding:0.7rem 2rem;border-radius:8px;font-weight:700;text-decoration:none;font-size:1rem;transition:opacity 0.2s}
.b-hero a:hover{opacity:0.85}

.b-cols{display:grid;gap:var(--cols-gap,1.5rem)}
...
.b-card{background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.07);height:100%;display:flex;flex-direction:column}
.b-card img{width:100%;height:200px;object-fit:cover}
.b-card-body{padding:1.25rem;flex:1;display:flex;flex-direction:column}
.b-card-body h3{font-size:1.1rem;font-weight:700;margin-bottom:0.5rem}
.b-card-body p{color:#64748b;font-size:0.9rem;line-height:1.6;flex:1}
.b-card-body a{display:inline-block;margin-top:0.875rem;background:#2563eb;color:#fff;padding:0.45rem 1rem;border-radius:6px;text-decoration:none;font-size:0.85rem;font-weight:600;align-self:flex-start}

.b-quote{border-left:4px solid #2563eb;border-radius:0 8px 8px 0;padding:1.25rem 1.25rem 1.25rem 1.75rem}
.b-quote p{font-size:1.05rem;font-style:italic;line-height:1.75;margin-bottom:0.75rem}
.b-quote footer{font-size:0.85rem;color:#64748b}

.b-social{display:flex;flex-wrap:wrap;align-items:center}
.b-social a{display:inline-flex;align-items:center;justify-content:center;border-radius:50%;text-decoration:none;transition:transform 0.2s,box-shadow 0.2s;color:#fff;flex-shrink:0}
.b-social a:hover{transform:scale(1.12);box-shadow:0 4px 14px rgba(0,0,0,0.2)}
.b-social a svg{display:block}

/* ── Post list (home/pages) ── */
.p-list{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.25rem}
.p-card{background:#fff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden;text-decoration:none;color:inherit;transition:transform 0.2s,box-shadow 0.2s;display:flex;flex-direction:column}
.p-card:hover{transform:translateY(-3px);box-shadow:0 8px 28px rgba(0,0,0,0.1)}
.p-card img{width:100%;height:180px;object-fit:cover;display:block}
.p-card-body{padding:1.1rem;flex:1;display:flex;flex-direction:column}
.p-card-body h2{font-size:1.1rem;font-weight:700;margin-bottom:0.3rem;color:#1e293b}
.p-card-body .date{font-size:0.78rem;color:#94a3b8;margin-bottom:0.5rem}
.p-card-body p{font-size:0.88rem;color:#64748b;line-height:1.6;flex:1}
.p-card-body .more{margin-top:0.75rem;font-size:0.82rem;font-weight:600;color:#2563eb}

/* ── Responsive breakpoints ── */
@media(max-width:768px){
  .s-links{display:none}
  .s-burger{display:block}
  .p-wrap{margin:0.75rem;border-radius:10px}
  .p-inner{padding:var(--page-pad-sm,1.25rem)}
  .b-cols.c2,.b-cols.c3,.b-cols.c4{grid-template-columns:1fr}
  .b-hero{min-height:60vw}
  .b-hero h1{font-size:clamp(1.4rem,6vw,2rem)}
  .p-list{grid-template-columns:1fr}
  .b-social{justify-content:center}
}
@media(max-width:480px){
  .s-nav{padding:0 1rem}
  .p-inner{padding:1rem}
  .b-hero-inner{padding:1.25rem}
}
</style>`;
...
(function(){
  ...
  // Mobile nav toggle
  var burger=document.querySelector('.s-burger');
...
// ─── Build wrapper style from block-level style props ─────────────────────────
  ...
  switch (type) {
    ...
    case 'social': {
      ...
      const icons = links.map(l => {
        ...
        const inner = l.url
          ? `<a href="${ea(l.url)}" target="_blank" rel="noopener noreferrer" class="b-social-icon" title="${esc(p.label||l.platform)}" style="width:${sz}px;height:${sz}px;background:${p.color}" class="b-social">${svgResized}</a>`
...
<div class="b-hero" style="${bg}min-height:${minH}">
  ...
  ${content.author?`<footer style="font-size:0.85rem;opacity:0.65;font-style:normal;font-weight:700">— <strong>${esc(content.author)}</strong>${content.role?`، ${esc(content.role)}`:''}</footer>`:''}
    ...
    // ── Columns ── (full-bleed when background is set)
    case 'columns': {
...
// ─── Published page shell (with nav, footer, SEO, theme) ─────────────────────
function shell({ title, siteTitle, siteTagline, excerpt, pageColor, pagePadding, body, siteTheme, coverImage }) {
  const padSm = pagePadding && pagePadding.includes('rem') ? '1.25rem' : '1rem';
...
(function(){
  ...
  var coverH = cover ? cover.offsetHeight : 0;
  ...
  if (img) {
    ...
    if (img.complete) extractColor(img);
    else img.addEventListener('load', function(){ extractColor(img); });
  }
  ...
  window.addEventListener('scroll', function(){
    ...
    if (img) img.style.transform = 'translateY(' + (scrollY * 0.4) + 'px)';
  }, { passive: true });
})();
</script>` : '';
  ...
  const fontUrl = 'https://fonts.googleapis.com/css2?family=Cairo:wght@400;500;600;700;800;900&family=Inter:wght@400;500;600;700;800&display=swap';
...
/* ══ THEMES ══ */
body.theme-minimal{background:#f5f5f5;color:#111}
body.theme-minimal{--surface:rgba(0,0,0,0.03);--border:rgba(0,0,0,0.08);--text-muted:#555}
...
.b-card-body h3{font-size:1.15rem;font-weight:700;line-height:1.4;color:inherit}
.b-card-body p{color:var(--text-muted);font-size:0.93rem;line-height:1.7;flex:1}
.b-card-body a{display:inline-block;margin-top:1rem;background:var(--accent);color:#111;padding:0.55rem 1.3rem;border-radius:6px;font-size:0.88rem;font-weight:700;align-self:flex-start;letter-spacing:0.02em;transition:background 0.18s,transform 0.18s}
.b-card-body a:hover{background:var(--accent-hover);transform:translateY(-1px)}
...
/* ══ SECTION with BG ══ */
.section-bleed{width:100vw;position:relative;left:50%;transform:translateX(-50%);padding:3.5rem 0}
.section-bleed .bleed-inner{max-width:1100px;margin:0 auto;padding:0 2rem}

/* ══ RESPONSIVE ══ */
@media(max-width:1024px){
  .b-cols.c4{grid-template-columns:repeat(2,1fr)}
  .b-gal-4{grid-template-columns:repeat(2,1fr)!important}
  .b-stats-4{grid-template-columns:repeat(2,1fr)!important}
}
@media(max-width:768px){
  .page-wrap{padding:2.5rem 1.25rem}
  .b-cols.c2,.b-cols.c3,.b-cols.c4{grid-template-columns:1fr}
  .b-hero{min-height:75vw}
  .b-hero-inner{padding:3rem 1.25rem}
  .b-hero h1{font-size:clamp(1.8rem,7vw,2.8rem)}
  .b-social{justify-content:center}
  .cb{margin-bottom:2rem}
  .section-bleed{padding:2.5rem 0}
...
// Root → redirect to first published page, or show 404
router.get('/', async (req, res) => {
  const first = await get("SELECT slug FROM posts WHERE status='published' ORDER BY published_at DESC LIMIT 1");
  if (first) return res.redirect(302, `/p/${first.slug}`);
  res.status(404).setHeader('Content-Type','text/html; charset=utf-8').send(notFound('لا توجد صفحات منشورة بعد'));
});

// Unified route: /p/:slug works for both pages and posts
router.get('/p/:slug', async (req, res) => {
  const post = await get("SELECT * FROM posts WHERE slug=? AND status='published'", [req.params.slug]);
  if (!post) return res.status(404).setHeader('Content-Type','text/html; charset=utf-8').send(notFound('الصفحة غير موجودة'));
  ...
  res.send(shell({ title:post.title, siteTitle:s.site_title, siteTagline:s.site_tagline, excerpt:post.excerpt, pageColor:meta.content?.pageColor||meta.pageColor||'#0d0d0d', pagePadding:meta.content?.pagePadding||meta.pagePadding||'3rem 2rem', body, siteTheme:s.site_theme, coverImage }));
});

// Legacy routes — keep working if old links were shared
router.get('/post/:slug', async (req, res) => res.redirect(301, `/p/${req.params.slug}`));
router.get('/page/:slug', async (req, res) => res.redirect(301, `/p/${req.params.slug}`));
